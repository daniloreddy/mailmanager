    ...
    with base_layout("Settings"):
        with ui.column().classes("full-width").style("padding:1.25rem; gap:1rem;"):
            ...
            with ui.card().classes("full-width q-pa-md"):
                ...
                spam_enabled = ui.checkbox("Enabled", value=config.get_bool("SPAM_ENABLED"))
                with ui.grid(columns=2).classes("full-width q-mt-sm").style("gap:12px;"):
                    ...
                    spam_port = ui.number(
                        "Port", value=config.get_int("SPAM_PORT", 783), min=1, max=65535
                    )
                    spam_user = ui.input("User (optional)", value=config.get("SPAM_USER", ""))
            ...
            with ui.card().classes("full-width q-pa-md"):
                ...
                sched_enabled = ui.checkbox("Enabled", value=config.get_bool("SCHEDULER_ENABLED"))
            ...
            with ui.card().classes("full-width q-pa-md"):
                ...
                log_level = ui.select(
                    _LOG_LEVELS, value=config.get("LOG_LEVEL", "INFO"), label="Level"
                )
            ...
            with ui.card().classes("full-width q-pa-md"):
                ...
                ui_refresh_enabled = ui.checkbox(
                    "Auto-refresh dashboards", value=config.get_bool("REFRESH_ENABLED")
                )
