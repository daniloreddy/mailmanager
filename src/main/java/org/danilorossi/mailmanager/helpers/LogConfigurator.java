...
public class LogConfigurator {
  ...
  protected static StreamHandler LOG_FH;

  static {
    try {
      val logFile = new File(FileSystemUtils.getExecutableDirectory(), LOG_FILE);
      LOG_FH = new FileHandler(logFile.getAbsolutePath(), 1_000_000, 5, true); // 1MB, 5 file
    } catch (IOException ignored) {
      System.err.println(
          "Impossibile determinare la directory dell'eseguibile. Uso la directory corrente: "
              + ignored.getMessage());
      LOG_FH = new ConsoleHandler();
    }
