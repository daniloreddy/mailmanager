...
public final class MailTextExtractor {
  ...
  private static boolean emptyString(final String content) {
  ...
  private static void extractPart(final Part part, @NonNull final StringBuilder out)
    ...
    if (part.isMimeType("text/plain")) {
      ...
      if (!emptyString(txt)) out.append(cleanText(txt)).append("\n\n");
      return;
    }

    if (part.isMimeType("text/html")) {
      val html = getTextPayload(part);
      if (!emptyString(html)) out.append(htmlToPlainText(html)).append("\n\n");
      return;
    }
    ...
    if (part.isMimeType("multipart/alternative")) {
      ...
      if (content instanceof Multipart mp) {
        ...
        if (!emptyString(best)) out.append(best).append("\n\n");
      }
      return;
    }
  ...
  private static boolean isAttachment(@NonNull final Part p) {
    try {
      ...
      if (!emptyString(disp) && disp.equalsIgnoreCase(Part.ATTACHMENT)) return true;
      // consideriamo attachment anche se c'è un filename
      if (!emptyString(p.getFileName())) return true;
    } catch (MessagingException ignored) {
    }
    return false;
  }
  ...
  private static String readAll(@NonNull final InputStream is, @NonNull final  Charset cs) throws IOException {
