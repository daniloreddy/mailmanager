    ...
    def rules_table() -> None:
        ...
        with ui.card().classes("w-full"):
            with ui.row().classes("justify-between items-center q-mb-md"):
                ui.label("Rules").classes("text-h6 text-grey-3")
            ...
            for r in rules:
                with ui.row().classes("items-center q-px-sm q-py-xs full-width").style(
                    ...
                    ui.label(r.imapConfigName).style(
                        "width:130px; font-weight:600; color:var(--text);"
                    )
                    ui.label(r.conditionSubject.value).style(
                        "width:90px; color:var(--muted);"
                    )
                    ui.label(r.conditionOperator.value).style(
                        "width:120px; color:var(--muted);"
                    )
                    ui.label(r.conditionValue).style("flex:1; color:var(--text);")
                    ui.label(r.actionType.value).style(
                        "width:110px; color:var(--muted);"
                    )
                    ui.label(r.destValue or "—").style(
    ...
    async def open_dialog(
        ...
        with ui.dialog() as dialog, ui.card().style("width:560px;"):
            ...
            with ui.grid(columns=2).classes("w-full gap-3"):
                ...
                dest_inp = ui.input(
                    "Destination / Label", value=r.destValue
                )
