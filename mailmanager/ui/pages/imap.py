...
def _refs_to_imap(refs: dict[str, Any]) -> ImapConfig:
    ...
    return ImapConfig(
        ...
        inboxFolder=str(refs["inboxFolder"].value or "INBOX"),
        username=str(refs["username"].value or ""),
        password=str(refs["password"].value or ""),
        ssl=bool(refs["ssl"].value),
        auth=bool(refs["auth"].value),
        useSpamAssassin=bool(refs["useSpamAssassin"].value),
        spamAction=SpamAction(refs["spamAction"].value),
        spamFolder=str(refs["spamFolder"].value or "Junk"),
        connectionTimeoutMs=_int(refs["connectionTimeoutMs"].value),
        readTimeoutMs=_int(refs["readTimeoutMs"].value),
        writeTimeoutMs=_int(refs["writeTimeoutMs"].value),
        smtpHost=str(refs["smtpHost"].value or ""),
        smtpPort=str(refs["smtpPort"].value or "587"),
        smtpUsername=str(refs["smtpUsername"].value or ""),
        smtpPassword=str(refs["smtpPassword"].value or ""),
        smtpSsl=bool(refs["smtpSsl"].value),
        smtpAuth=bool(refs["smtpAuth"].value),
    )
...
async def imap_page() -> None:
    ...
    def imap_table() -> None:
        ...
        with ui.card().classes("full-width"):
            ...
            if not configs:
                ui.label("No configurations").classes(
