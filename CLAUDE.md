...
app/
  main.py                Entrypoint AND FastAPI app — module-level `app = FastAPI(...)`, no separate
                         ...
                         GET / — redirects to /ui/; the auth gate only intercepts /ui/* itself
                         (see Key invariants), so the redirect always succeeds and unauthenticated
                         visitors land on /login only once they hit /ui/.
...
- `verify_password()` (redberry-webkit ≥v0.2.0, scrypt N=131072, ~150-250ms/~128MB) runs via `asyncio.to_thread(...)` in `POST /auth/login` (`app/ui/router.py`) — never inline, or it blocks the event loop for the whole window on every login attempt
- `AUTH_SECURE_COOKIE` (env, default `0`) forces the session cookie's `Secure` flag when TLS terminates somewhere that doesn't set `X-Forwarded-Proto: https`; otherwise `is_secure_context(request.headers)` decides
- Background tasks (`purge_task`, `config_task` in `app/main.py`'s `lifespan`) have `add_done_callback(_crash_on_task_error)` — an unhandled exception crashes the process (`os._exit(1)`) instead of dying silently and leaving rate-limit purging / config hot-reload broken
...
- Auth on `/ui` is unconditional — cookie session via `AuthManager` always gates the whole dashboard, no opt-out. `HOST` (default `127.0.0.1`, non-Docker runs only — read directly via `os.environ`, see `app/main.py`) is a separate, independent deployer choice about which interface the server binds: e.g. `HOST=0.0.0.0` on a trusted LAN, or `HOST=127.0.0.1` (default) behind a Cloudflare/SSH tunnel exposed to the internet — either way, login is required. In Docker, `HOST=0.0.0.0` is fixed unconditionally inside the container (container's own `127.0.0.1` is unreachable from the host — Docker-networking necessity, not a deployer choice, hardcoded in the Dockerfile's `CMD`); external reachability there is controlled separately by `HOST` on the Compose `ports:` mapping in `docker-compose.yml` (same env var name, different layer — see `docker.md` §5). If this invariant ever regresses (a `REQUIRE_AUTH`-style toggle reintroduced to make auth optional), it's a bug — see `fastapi-auth.md` in global guidelines: "there is no REQUIRE_AUTH-style toggle... the cookie gate is always there"
- The `_auth_gate` middleware (canonical constants `_UI_PREFIX`/`_LOGIN_PATHS`/`_UI_BYPASS_PREFIXES` in `app/main.py`, matching `redberry-webapp-template`'s current `main.py.jinja`) only intercepts paths under `_UI_PREFIX` (`/ui`) — `/health`, `/`, `/login`, `/auth/*` are outside its scope entirely, not exceptions carved out of a blanket gate. NiceGUI is mounted at `/ui` (`ui.run_with(app, mount_path="/ui", ...)`), so `/ui/*` is the cookie-protected surface. `_UI_BYPASS_PREFIXES` excludes `/ui/_nicegui` (verified at runtime against the installed NiceGUI 3.13.0: covers both `/ui/_nicegui_ws` websocket and `/ui/_nicegui/<version>/...` static/library/component assets) from the cookie middleware — re-verify this prefix if the pinned NiceGUI version changes, since it's not guaranteed stable across major versions. This now matches the template's own value (`/ui/_nicegui`) — an earlier version of this doc flagged a deliberate deviation from a template using `/ui/socket.io`, which no longer applies since the template was fixed upstream
...
- `/health` is public simply by being outside `_UI_PREFIX` — no separate allowlist needed
